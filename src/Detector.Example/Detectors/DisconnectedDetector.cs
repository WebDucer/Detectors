...
namespace Detector.Example.Detectors {
  public class DisconnectedDetector<T> : DetectorBase<T> {
    ...
    private readonly static TimeSpan _DEFAULT_TIMEOUT_TO_FIRST_EVENT = TimeSpan.FromSeconds(1);

    public DisconnectedDetector(ConnectStateEvent connectEvent, ResultState positiveState = ResultState.Success, T positiveResult = default(T))
